...
sudo yum update
sudo yum remove aspnetcore-runtime-2.1 dotnet-sdk-2.1
sudo yum install aspnetcore-runtime-2.2 dotnet-sdk-2.2
...
DOWNLOADER_BIN=$NIRVANA_ROOT/bin/Release/netcoreapp2.0/Downloader.dll
NIRVANA_TAG=v3.5.0
...
if [ ! -d $NIRVANA_ROOT ]
then
    sudo git clone https://github.com/Lucioric2000/Nirvana.git
    #sudo git checkout $NIRVANA_TAG
    sudo git checkout merger
fi
...
if [ ! -f $REF_TEST ]
then
    sudo_create_dir $DATA_DIR
    echo dotnet $DOWNLOADER_BIN --ga $GENOME_ASSEMBLY --out $DATA_DIR
    sudo dotnet $DOWNLOADER_BIN --ga $GENOME_ASSEMBLY --out $DATA_DIR
fi
...
then
    sudo wget https://github.com/samtools/htsjdk/raw/master/src/test/resources/htsjdk/variant/HiSeq.10000.vcf
fi
